...
plt.plot(predictions_series_test.index, predictions_series_test, label='Predicted (Test)')

plt.legend()
plt.show()

# 计算测试集预测数据的残差
forecast_residuals = test['seasonal'] - predictions_series_test

# 计算MAE（排除缺失值）
mae = np.mean(np.abs(forecast_residuals.dropna()))
